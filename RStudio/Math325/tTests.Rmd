...
(2) The sampling distribution of the sample mean $\bar{x}$ *can be assumed to be normal*. This is a safe assumption when either (a) the population data can be assumed to be normally distributed using a [Q-Q Plot](./Analyses/NormalProbabilityPlots.html) or (b) the size of the sample (n) that was taken from the population is large (at least n > 30).
...
(2) The sampling distribution of the sample mean of the differences $\bar{d}$ ($\bar{x}$ of the differences) can be assumed to be normal. (This second requirement can be assumed to be satisfied when (a) the differences themselves can be assumed to be normal from a [Q-Q Plot](./Analyses/NormalProbabilityPlots.html), or (b) when the sample size $n$ of the differences is large.)
...
(2) The sampling distribution of the difference of the sample means $(\bar{x}_1 - \bar{x}_2)$ *can be assumed to be normal*. (This is a safe assumption when the sample size of each group is $30$ or greater or when the population data from each group can be assumed to be normal with a [Q-Q Plot](./Analyses/NormalProbabilityPlots.html).)
